...
class ExtractionAttack:
    ...
    def __init__(
        ...
        # Labelled once, not per round: the set never changes.
        self._eval_y: Optional[torch.Tensor] = None
    ...
    def run(self, verbose: bool = True) -> ExtractionResult:
        ...
        for rnd in range(self.cfg.n_rounds):
            ...
            if self._eval_y is None:
                self._eval_y = self.oracle.query(self._eval_x)
            fidelity  = self.trainer.evaluate_fidelity(
                self.oracle.query, self._eval_x, oracle_out=self._eval_y
            )
